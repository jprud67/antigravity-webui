...
### 📌 Sprint 21 (v0.2.25 – Token Economy & Context Optimization Architecture)
| Milestone | Description | Owner | Target | Status |
|---|---|---|---|---|
...
| **Compactage Proactif Automatique & Élagage d'Historique** | Exécution automatique du compactage in-place avant chaque appel au CLI `agy` : remplacement systématique des sorties d'outils volumineuses (`view_file`, `git diff`, logs de build) des tours antérieurs par des badges d'une ligne ultra-légers dans `transcript.jsonl` (avec conservation intégrale dans `transcript_full.jsonl`). Troncature post-tour inconditionnelle (`max_lines=30`, `max_chars=2500`). Élimination du bloat de contexte cumulatif sur les sessions longues. | Storage / Driver | 2027‑02‑18 | ✅ **Completed** |
| **Optimisation de l'Injection Workspace & Mode Éco par Défaut** | Suppression de la réinjection doublon `--add-dir` lorsque le workspace correspond déjà au répertoire de travail `cwd`. Activation du mode Éco par défaut (`ecoMode: true` dans les paramètres et dans `ChatInput`) avec directives de sobriété de tokens systématiques. | Core / Frontend | 2027‑02‑18 | ✅ **Completed** |
